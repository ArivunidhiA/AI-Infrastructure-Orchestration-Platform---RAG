...
// White color configuration - using white hue (0 saturation, high lightness)
const glowColorMap = {
  ...
  white: { base: 0, spread: 0 } // White - no hue variation
};
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  children, 
  className = '', 
  glowColor = 'white', // Default to white
  size = 'md',
  width,
  height,
  customSize = false
  ...
  useEffect(() => {
    ...
    // Only track pointer when hovering over this specific card
  ...
  const getInlineStyles = () => {
    // For white color, use 0 saturation and high lightness
    const isWhite = glowColor === 'white';
    const saturation = isWhite ? 0 : 100;
    const lightness = isWhite ? 100 : 70;
    ...
    const baseStyles: React.CSSProperties & Record<string, any> = {
      ...
      '--hue': isWhite ? '0' : 'calc(var(--base) + (var(--xp, 0) * var(--spread, 0)))',
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 0) ${saturation}% ${lightness}% / var(--bg-spot-opacity, 0.15)), transparent
      )`,
  ...
  const beforeAfterStyles = `
    ...
    [data-glow]::before {
      background-image: radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 0) var(--saturation, 0)% var(--lightness, 100)% / var(--border-spot-opacity, 0.8)), transparent 100%
      );
      filter: brightness(2);
    }
